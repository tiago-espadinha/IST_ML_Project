...
def main():
    ...
    _, x_cluster1_scaled, x_cluster2_scaled, y_cluster1_scaled, y_cluster2_scaled, inlier_mask = ransac_classifier(X_train_scaled, y_train_scaled.ravel(), 0.4)

    print('x_cluster1.shape\t= ', x_cluster1_scaled.shape)
    print('y_cluster1.shape\t= ', y_cluster1_scaled.shape)
    print('x_cluster2.shape\t= ', x_cluster2_scaled.shape)
    print('y_cluster2.shape\t= ', y_cluster2_scaled.shape)

    regr1 = LinearRegression().fit(x_cluster1_scaled, y_cluster1_scaled)

    regr2, x_cluster2_scaled, _, y_cluster2_scaled, _, _ = ransac_classifier(x_cluster2_scaled, y_cluster2_scaled.ravel(), 0.5)
    ...
    MSE1, _, max_MSE1 = cv_metrics(regr1, x_cluster1_scaled, y_cluster1_scaled.ravel(), x_cluster1_scaled.shape[0])
    MSE2, _, max_MSE2 = cv_metrics(regr2, x_cluster2_scaled, y_cluster2_scaled.ravel(), x_cluster2_scaled.shape[0])
